...
public interface IFreecellController<K> {
   ...
   * To do so, it should ask the provided model to start a game with the provided parameters, and
   * then “run” the game in the following sequence until the game is over:
   ...
   * asked one at a time:
   * The source pile (e.g., "C1", as a single word). The pile number begins at 1, so that it is more
   * human-friendly.
   ...
   * @throws IOException if the controller is unable to successfully receive input or transmit output
   */
